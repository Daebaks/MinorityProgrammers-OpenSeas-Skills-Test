...
const SingleAsset = () => {
    ...
    useEffect(() => {
        let newAs;
        fetch(`https://api.opensea.io/api/v1/asset/${asset_token_address}/${asset_token_id}/`, {
            method: 'GET',
            mode: 'cors',
            headers: {
    ...
    return (
        <>
            {myAssetObj.length !== 0 ? <table className='wrap'>
                <tbody>
                    <tr>
                        <td>
                            <Banner
                            ...
                            <div className='single-asset-wrapper'>
                                ...
                                <div className={
                                    (myAssetObj.asset_name_single && (myAssetObj.asset_name_single).length > 15) ? 'bid-asset-name-long' :
                                        'bid-asset-name-short'
                                }>
                                    {myAssetObj.asset_name_single}
                                </div>
                                <div className='bid-logo-frame'>
                                    <img src={myAssetObj.asset_logo_single} alt='' className='bid-asset-logo' />
                                </div>
                                <div className='asset-bid-info'>
                                    <div className='bid-ends'>Bid Ends in</div>
                                    <div className='expire-bid-time'>{(myAssetObj.asset_bid_ends).substring(0, 10)}</div>
                                    <div className='current-bid'>Current Bid<br />${myAssetObj.asset_current_bid}</div>
                                    <div className='starting-bid'>Starting Bid<br />${myAssetObj.asset_starting_bid}</div>
                                </div>
                                <form onSubmit={handleBidding}>
                                    ...
                                    />
                                    <input className='bidding-clickable' type='submit' value='Place Bid' />
                                </form>
                                <div className='asset-desc'>
                                    ...
                                    <div className='bid-details-data'>
                                        {myAssetObj.asset_contract_address_single}<br />
                                        {myAssetObj.asset_token_id}<br />
